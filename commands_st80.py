...
inspection_xt = "commit,Inspection,XT,PX,500,475,525,Numeric,mm,1,0,1,PY,500,475,525,Numeric,mm,1,0,2,CPC,400,395,405,Numeric,mm,1,0,3,CPE,400,395,405,Numeric,mm,1,0,4,CAD,1,1,1,Boolean,DBU,1,0,5,1/"
inspection_vs ="commit,Inspection,VS,M,1,0,0,Numeric,N,1,0,L,1,0,0,Numeric,mm,1,0,A,1,0,0,Numeric,mm,1,0,1/"
...
def commit(cadena, name_piece):
    ...
    match options[1]:
        ...
        case "Inspection":
            ...
            if options[2] == "VS":
                if len(options) == 29:
                    if "M" in options and "L" in options and "A" in options:
                        startM = options.index("M")
                        endM = options.index("M")+8
                        startPx = options.index("L")
                        endPx = options.index("L")+8
                        startPy = options.index("A")
                        endPy = options.index("A")+8
                            
                        if endM != "L" and endM != "A":
                            measurement = options[startM:endM]
                            # print(measurement)
                        else:
                            return "FAILED", []

                        if endPx != "M" and endPx != "A":
                            position_x = options[startPx:endPx]
                            # print(position_x)
                        else:
                            return "FAILED",[]
                                
                        if endPy != "M" and endPy != "L":
                ...
                if len(options) == 50:
                    if "PX" in options and "PY" in options and "CPC" in options and "CPE" in options and "CAD" in options:
                        ...
                        endPx2 = options.index("PX")+9
                        startPy2 = options.index("PY")
                        endPy2 = options.index("PY")+9
                        startCPC = options.index("CPC")
                        endCPC = options.index("CPC")+9
                        startCPE = options.index("CPE")
                        endCPE = options.index("CPE")+9
                        startCAD = options.index("CAD")
                        endCAD = options.index("CAD")+9
