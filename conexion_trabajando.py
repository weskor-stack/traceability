...
def parameters_electrical(element, name_piece):
    ...
    try:
        ...
        value, low_limit, high_limit, data_type, units, result, metadata = element[1:8]
        compoperator = evaluation.evaluation(element[1:4])
        description = f"{station[1]} {measurement[1]} Test"
...
def duration_json(station_id, part_id):
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT taskresult, tasktimestamp, taskduration, metadata FROM duration WHERE station_id = %s AND part_id = %s LIMIT 1",
                (station_id, part_id)
            )
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute('''
                ...
                WHERE part_id = %s
                ORDER BY inspection_measurement_id DESC
                LIMIT 4
            ''', (part_id,))
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute('''
                ...
                WHERE part_id = %s
                ORDER BY pressfit_measurement_id ASC
        ...
        with conn.cursor() as cursor:
            cursor.execute('''
                SELECT 
                    ...
                    result,
                    unit, 
                    description, extra1, extra2, compoperator,
                    create_registration
...
def inspection_data3(part_id,limite):
    try:
        with conn.cursor() as cursor:
            cursor.execute('''
                SELECT 
...
def parameters_continuity(element):
    ...
    # Preparar evaluación
    evaluacion = [element[3], element[4], element[7]]
    ...
    try:
        ...
        sql = '''
            INSERT INTO parameters_leaktest (
                ...
                extra1, extra2, compoperator, station_id, part_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        '''
