...
def xml_file(limite):
    ...
    try:
        ...
        if type_station == 1:  # Screwing
            ...
            add_complete_xml(tests_element, piece_id, station_name, limite)
...
def add_screwing_xml(parent, piece_id, station_name, limite):
    ...
    for x in screwing:
        ...
        ET.SubElement(test, "Description").text = safe_str(x[10]) + f" STEP {indice}"
        ...
        ET.SubElement(test, "Result").text = safe_str(x[6])
...
def add_pressfit_xml(parent, piece_id, station_name, limite):
    ...
    for x in pressfit:
        ...
        ET.SubElement(test, "LowerLimit").text = safe_str(x[2])
        ET.SubElement(test, "UpperLimit").text = safe_str(x[3])
        ET.SubElement(test, "DwellTime").text = safe_str(x[12])

def add_inspection_xml(parent, piece_id, station_name, limite):
...
def add_complete_xml(parent, piece_id, station_name, limite):
    """Añade todos los tipos de tests al XML para estación completa"""
    # Screwing
    add_screwing_xml(parent, piece_id, station_name, limite)
    
    # Pressfit
    add_pressfit_xml(parent, piece_id, station_name, limite)
    
    # Inspection
    add_inspection_xml(parent, piece_id, station_name, limite)
    ...
    for x in continuity:
        ...
        ET.SubElement(test, "Number").text = str(indice)
        ET.SubElement(test, "Name").text = f"{indice}b. {station_name} Test"
        ET.SubElement(test, "Description").text = safe_str(x[1]) + f" STEP {indice}"
        ...
        ET.SubElement(test, "Unit").text = safe_str(x[5])
        ET.SubElement(test, "Status").text = safe_str(x[6])
        ET.SubElement(test, "Value").text = safe_str(x[7])
        ET.SubElement(test, "DefectCode").text = safe_str(x[8])
        indice += 1
    ...
    for x in leak:
        ...
        ET.SubElement(test, "Number").text = str(indice)
        ET.SubElement(test, "Name").text = f"{indice}b. {station_name} Test"
        ET.SubElement(test, "Description").text = safe_str(x[5]) + f" STEP {indice}"
        ET.SubElement(test, "TrialPeriod").text = safe_str(x[1])
        ET.SubElement(test, "Value").text = safe_str(x[2])
        ET.SubElement(test, "Result").text = safe_str(x[3])
        ET.SubElement(test, "Leak").text = safe_str(x[4])
        ET.SubElement(test, "LowLimit").text = safe_str(x[6])
        ET.SubElement(test, "HighLimit").text = safe_str(x[7])
        indice += 1
    ...
    for x in welding:
        ...
        ET.SubElement(test, "Number").text = str(indice)
        ET.SubElement(test, "Name").text = f"{indice}b. {station_name} Test"
        ET.SubElement(test, "Description").text = safe_str(x[4]) + f" STEP {indice}"
        ET.SubElement(test, "WeldingTime").text = safe_str(x[1])
        ET.SubElement(test, "WeldingPower").text = safe_str(x[2])
        ET.SubElement(test, "CollapseDistance").text = safe_str(x[3])
        ET.SubElement(test, "Result").text = safe_str(x[5])
...
def save_xml_file(root, filename):
    ...
    try:
        ...
        file_data_bk = f"C:/AMC/RETRY/XML/{today_year}/{today_month}/{today_day}/"
